...
| 모드 | 크기 규칙 | 결과물 |
| --- | --- | --- |
| `top-bottom` | 원본 적응형 | 위에는 전체 원본, 아래에는 007 손글씨 관찰 도감. 두 패널이 원본 크기를 유지하며 정확히 50/50 |
| `left-right` | 원본 적응형 | 왼쪽에는 전체 원본, 오른쪽에는 007 손글씨 관찰 도감. 두 패널이 원본 크기를 유지하며 정확히 50/50 |
| `design-only` | 원본 적응형 | 원본 사진은 보이지 않고 변환 디자인만 표시. 원본 비율과 크기 유지 |
| `wallpaper-pack` | 기기별 네 가지 크기 | 휴대전화, iPad, 데스크톱, 어린이용 스마트워치 PNG 개별 출력 |

정확한 사용자 픽셀 > 지정 비율 또는 용도 > 일반 모드의 원본 적응형 순서입니다. 원본 `007.md`의 3:4는 초기 창작 캔버스일 뿐 현재 Skill의 숨은 기본값이 아닙니다.
...
- **독립형 네 작품:** 각 기기는 원본 사진만 참조하며 관찰 조각, 읽기 순서, 종이 리듬, 색의 초점과 주석 조합을 각각 탐색합니다.

연결형은 크롭이 아닙니다. 네 장 모두 따로 생성하고 구성하며 검수하고, iPad→휴대전화→데스크톱→스마트워치 순으로 참조를 이어 붙이지 않습니다.

## 문구는 보기와 읽기를 연결해야 합니다

생성 전에 자동 문구, 사용자 지정 문구, 무문자 출력 중 하나를 선택합니다. 문구가 있으면 대상 언어나 지역도 지정합니다.

자동 문구는 보이는 재료, 동작, 기능, 관계, 근거 있는 감각 단서나 알려진 맥락에서 필요한 만큼의 짧은 단어, 문장, 의성어와 주석을 만듭니다. 감각과 기억 표현에는 이미지 증거나 사용자 정보가 필요합니다.

고정 개수는 없습니다. 각 관찰 조각에 실제 정보 가치가 있을 때만 글을 붙이고 번호, 날짜, 서명이나 기억을 지어내지 않습니다.
...
이미지 모델이 같은 출처의 관찰 조각, 실물 재료감, 느슨한 종이 리듬, 가는 검정 손글씨, 따뜻한 색과 스캔 종이결을 만듭니다. `scripts/compose_panel.py`는 캔버스 계획, 정확한 50/50 래스터 합성, 최종 크기와 검수만 수행합니다.
...
- [Skill 워크플로](SKILL.md)
- [중문 전체 프롬프트](references/xxd-panel-007-prompt.zh-CN.md)
- [영문 전체 프롬프트](references/xxd-panel-007-prompt.en.md)
