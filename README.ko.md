<p align="center">
  <img src="./assets/banner.svg" alt="XXD Panel 007 프로젝트 배너" width="1200">
</p>
...
[![Four Modes](https://img.shields.io/badge/Modes-4-d75d32?style=flat-square)](#네-가지-출력-하나의-관찰-도감-논리)
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 007 예시 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 007 예시 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 007 예시 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 007 예시 8"></td>
  </tr>
</table>

## 추가 3:4 상하 구성 예시

위 16:9 예시와 같은 네 개의 독립 소스를 완전한 3:4 상하 캔버스로 다시 생성했습니다. 위쪽은 현실 사진을 유지하고 아래쪽은 Panel 007의 원본 프롬프트에 따라 재구성합니다. 영어 문구는 각 사진의 내용·분위기·함의에서만 생성했으며 모든 결과를 개별 검수했습니다.

<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 007 추가 상하 예시 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 007 추가 상하 예시 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 007 추가 상하 예시 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 007 추가 상하 예시 4"></td>
  </tr>
</table>
...
> 실물 미니어처 · 확대／단면／반복 · 엇갈린 여백 · 가는 검정 손글씨 · 스캔 종이결

XXD Panel 007는 Codex와 호환 에이전트를 위한 이미지 생성 Skill입니다. 한 장의 사진을 실물 같은 작은 전체 모습, 확대, 단면, 반복 조각과 세부 파편으로 나누어 깨끗한 흰 종이의 위쪽과 중간에 느슨하게 배치합니다.
...
**Panel 007**은 원본 사진의 핵심을 보존하면서 다음과 같은 시각 방향을 원하는 작업에 적합합니다: **사진을 보며 읽는 친밀한 손글씨 생활 관찰 도감으로**. 먼저 용도와 원하는 결과를 확인한 뒤 구성, 크기, 텍스트와 전달 형식을 결정합니다.
...
> XXD Panel 007으로 이 이미지를 작업하고, 먼저 가장 어울리는 구성과 크기를 추천해 줘.
...
## 빠른 판단: XXD Panel 007이 나에게 맞을까요?

| 궁금한 점 | 이 스타일이 주는 답 |
|---|---|
| **얻게 되는 결과** | 사진을 보며 읽는 친밀한 손글씨 생활 관찰 도감으로 |
| **한눈에 알아볼 특징** | 실물 미니어처 · 확대／단면／반복 · 엇갈린 여백 · 가는 검정 손글씨 · 스캔 종이결 |
...
```text
원본 사실 고정 → 유용한 전체／확대／단면／반복／세부 선택 → 실물 재료 보존 → 흰 종이에 엇갈리게 배치 → 가는 검정 손글씨로 연결 → 스캔의 부드러움과 안료 흡수로 마감
```

무관한 사진으로 바꿔도 작은 삽화의 윤곽, 핵심 색면, 여백 관계와 문구가 거의 그대로 성립한다면 007가 아닙니다.

## 완성작에서 바로 알아볼 수 있는 스타일 특징

- **같은 출처의 관찰 조각:** 전체, 확대, 단면, 반복과 세부가 모두 한 주제에 속하며 고정 칸 수가 없습니다.
- **실물 메모 감각:** 작은 크기, 둥근 윤곽, 약한 원근과 손채색이 재료와 식별성을 보존합니다.
- **근거 있는 세부:** 알갱이, 그을린 가장자리, 부스러기, 섬유, 이음, 반사와 단면은 원본이 뒷받침할 때만 씁니다.
- **느슨한 읽기:** 위쪽과 중간에 엇갈려 배치해 가로 또는 문자 체계에 맞는 흐름을 만듭니다.
- **엄격한 그리드 금지:** 표, 카드 벽, 열, 표본 매트릭스, 같은 크기 아이콘이 아닙니다.
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 007의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-007 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-007 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-007 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-007 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-007 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-007 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 007 적용, 공통 설정은 한 번만 확정
/xxd-panel-007 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
Claude Code 사용자는 같은 폴더를 `~/.claude/skills/xxd-panel-007`에 연결할 수 있습니다. 설치 후 에이전트 세션을 다시 시작하세요.
...
- [Skill 워크플로](SKILL.md)
- [중문 런타임 어댑터](references/xxd-panel-007-prompt.zh-CN.md)
- [영문 런타임 어댑터](references/xxd-panel-007-prompt.en.md)
...
**사물을 줄 세우지 말고, 관찰한 시간 자체를 종이에 남기세요.**
