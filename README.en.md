...
The repository reserves [`assets/examples/`](assets/examples/) for future work. Only finished 007 artwork verified by the project owner will be added; no post or image from another style is used as a placeholder.

Future samples will demonstrate 007's adaptability. Their subjects, observation fragments, palette, copy, and canvas ratios will never become generation references or defaults.

## Four outputs, one observation-atlas logic
...
| --- | --- | --- |
| `top-bottom` | source-adaptive | complete source above, 007 handwritten observation atlas below; both panels retain the source size and split exactly 50/50 |
| `left-right` | source-adaptive | complete source left, 007 handwritten observation atlas right; both panels retain the source size and split exactly 50/50 |
...
Exact user pixels > explicit ratio or destination > source adaptation for ordinary modes. The original `007.md` used a 3:4 creative canvas, but that historical example is not a silent default in the current Skill.
...
- **Independent set:** every device references only the source photograph and may explore different observation fragments, reading orders, paper rhythms, colour focuses, and note combinations.
...
The image model creates source-bound observation fragments, physical material cues, loose paper rhythm, thin black handwriting, warm clean colour, and scanned-paper touch. `scripts/compose_panel.py` only plans canvases, performs exact 50/50 raster composition, finalises dimensions, and audits results. It never fakes artwork with programmatic drawing.
